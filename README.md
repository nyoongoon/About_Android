...
# Extra data(부가 데이터)
- 한 액티비티에서 다른 액티비티를 띄울 때 데이터를 전달해야하는 경우가 있음. 기본적으로 액티비티를 띄울 때 전달되는 인텐트 안에 Extra data(부가 데이터)를 넣어 전달하는 방법을 권장
- 인텐트 안에 Bundle 객체가 있는데, 번들 객체는 해시테이블과 유사해서 putExtra()와 get@@@Extra()메서드로 데이터를 넣거나 뺼 수 있음.
- 이렇게 번들 객체 안에 넣은 데이터를 부가 데이터라고 함.
- 전달하고 싶은 데이터가 객체 자료형인경우 바이트 배열로 변환하거나 Serializable 인터페이스를 구현하는 객체로 만들어 직렬화 한 다음 전달.
- 안드로이드에서는 Serializable 인터페이스와 유사한 Parcelable인터페이스를 권장 (크기가 더 작음). 
<br/><br/>

# Fragment (프레그먼트)
- 하나의 화면을 여러 부분으로 나눠서 보여주거나 각각의 부분 화면 단위로 바꿔서 보여주고 싶을 때 사용하는 것. 
- 액티비티처럼 독립적으로 동작하는 부분화면을 만들 때 사용.
- 화면들을 독립적으로 구성, 그 상태를 관리.
- 프래그먼트는 항상 액티비티 위에 올라가 있어야 함. 프래그먼트가 제대로 동작하는 시점은 메모리에 만들어진 시점이 아니라 액티비티에 올라가는 시점. 
- 인텐트로 데이터 전달X. 액티비티와 프래그먼트 간에 데이터를 전달할 때는 메서드 호출 방식 이용.
